...
// ==========================================
const ai = new GoogleGenerativeAI(process.env.GEMINI_API_KEY || "dummy-key-to-prevent-crash");
...
// FIX PORT : Utilisation de la variable standard de Render
...
Ton but est de :
1. Demander le motif de la consultation.
2. Proposer des créneaux en utilisant TOUJOURS tes outils. Ne sois JAMAIS créative avec les horaires. Propose toujours des alternatives proches si c'est plein.
3. Demander le Nom et Prénom complet du patient.
4. Valider le RDV dans la base via l'outil.
...
async function processMessageAI(phone, text) {
    ...
    try {
        ...
        // Gestion du Function Calling (Boucle)
        while (response.functionCalls() && response.functionCalls().length > 0) {
            ...
            // Envoyer les résultats des fonctions à Gemini pour la réponse finale
            const result2 = await model.generateContent({
                contents: [...currentContents, response.candidates[0].content, { role: "user", parts: toolResponses }],
                tools: tools
            });
...
app.post('/webhook', async (req, res) => {
  ...
  if (body.object === "whatsapp_business_account") {
    // Toujours renvoyer un statut 200 IMMÉDIATEMENT
    res.sendStatus(200);
    ...
    try {
      ...
      if (message && message.type === "text") {
        ...
        const aiReply = await processMessageAI(fromPhone, textBody);
        if (aiReply) {
